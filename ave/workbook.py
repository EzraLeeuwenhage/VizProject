...
#%%
pd.to_pickle(accident_df, 'accident.pkl.gz')
#%%
...
#%%
pd.to_pickle(merged, './merged.pkl.gz')

#%%
dfs['vehicle']['cost'] = 1
pd.to_pickle(dfs['vehicle'], 'vehicle.pkl.gz')
...
for sub in ['casualty', 'accident', 'vehicle']:
    ...
    for col in df.columns:
        ...
        for val in df[col]:
            type_name = type(val)
            if type_name in types.keys():
                types[type_name] += 1
            else:
                types[type_name] = 1
        print(f'{sub}, {col}: {types}')
